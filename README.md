...
open with a bare `Permission denied`:
...
### 3. Update to 2.0.0 — on the leader

An explicit `update-module` is not subject to `min-from`.

    api-cli run update-module --data '{"module_url":"ghcr.io/nethserver/dependencytrack:2.0.0","instances":["dependencytrack1"],"force":true}'

### 4. Restore the analyzers and start — on the node running the app
...
The script replays the v4 analyzer settings, which the migrator leaves empty:
